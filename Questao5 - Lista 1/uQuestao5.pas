...
begin
  ...
  if wQtde_Numeros < 1 then
     begin
       ShowMessage('Número inválido, digite novamente');
       Inc(wFinaliza1);
       if wFinaliza1 = 3 then
          begin
            ShowMessage('O programa será finalizado, simultânea digitação de numeros negativos(Ou ZERO)');
...
begin
  ...
  if wAux > 0 then
     begin
       ...
       if wI = wQtde_Numeros then
          begin
            ...
            ShowMessage('Maior Número: ' + IntToStr(wMaiorNumero) + ', Quantidade de vezes digitada: ' + IntToStr(wContador));
     ...
     begin
       ShowMessage('Número inválido, favor digitar novamente!');
       Inc(wFinaliza2);
     end;
